...
describe MyArtecChecker do
  ...
  describe '#check_user' do
    it 'should return false' do
      WebMock.enable!
      email = 'asdfasdf'
      stub_request(:get, "https://staging-booth-my.artec3d.com/users/exist.json?user%5Bemail%5D=#{email}").to_return(status: 200, body: { 'exist' => false }.to_json)
      expect(JSON.parse(checker.check_user(email))).to eq({ 'exist' => false })
    end
    it 'should return true' do
      WebMock.enable!
      email = 'malexeev@artec-group.com'
      stub_request(:get, "https://staging-booth-my.artec3d.com/users/exist.json?user%5Bemail%5D=#{email}").to_return(status: 200, body: { 'exist' => true }.to_json)
